...
const Internship: React.FC<Props> = ({ navigation }) => {
  ...
  const [renderPage, setRenderPage] = useState<Element>(<></>);
  ...
  // renderiza mensgaem de sem estagios ou mostra estagios do aluno
  useEffect(() => {
    ...
    } else if (internships[tabIndex] === undefined) {
      setRenderPage(
        <>
          <BiggerContainer>
            <Section>
              <TitleText>
                Ainda não há estágios adicionados! Adicione uma turma de estágio
                na aba Perfil
              </TitleText>
            </Section>
          </BiggerContainer>
  ...
  }, [internships, loadingInternship, tabIndex]);
